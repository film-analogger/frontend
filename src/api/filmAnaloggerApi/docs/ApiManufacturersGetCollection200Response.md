# ApiManufacturersGetCollection200Response

Manufacturer.jsonld-read-manufacturer_timestampable-blameable-read_translatable-read collection.

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**hydra_totalItems** | **number** |  | [optional] [default to undefined]
**hydra_search** | [**HydraCollectionBaseSchemaNoPaginationHydraSearch**](HydraCollectionBaseSchemaNoPaginationHydraSearch.md) |  | [optional] [default to undefined]
**hydra_view** | [**HydraCollectionBaseSchemaAllOfHydraView**](HydraCollectionBaseSchemaAllOfHydraView.md) |  | [optional] [default to undefined]
**hydra_member** | [**Array&lt;ManufacturerJsonldReadManufacturerTimestampableBlameableReadTranslatableRead&gt;**](ManufacturerJsonldReadManufacturerTimestampableBlameableReadTranslatableRead.md) |  | [default to undefined]

## Example

```typescript
import { ApiManufacturersGetCollection200Response } from './api';

const instance: ApiManufacturersGetCollection200Response = {
    hydra_totalItems,
    hydra_search,
    hydra_view,
    hydra_member,
};
```

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)
